...
En este contexto, este trabajo se propone trazar el camino de
...
A los efectos de avanzar en la construcción de evidencia se va a llevar
adelante un escalamiento a partir de los discursos parlamentarios y
...
El parlamento uruguayo no cuenta con un registro de las votaciones en
sala. Es decir, no hay un registro nominal de las votaciones. La
constitución uruguaya prevé dos situaciones en las que se debe registrar
nominalmente las votaciones. En primer lugar, un legislador cualquiera
puede solicitar que la votación de un asunto se haga nominal, lo cual
implica una votación no nominal para resolver la viabilidad de la
propuesta y luego se lleva adelante la votación nominal si es que la
votación fue afirmativa. El segundo momento en donde la votación debe
...
De este modo, se implementó este proceso para generar votaciones
nominales para poder iniciar el escalamiento con votaciones que
mantienen la misma distribución. En el siguiente apartado se presenta el
...
y en esos casos la escala resultante se puede invertir.
